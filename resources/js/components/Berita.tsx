...
export default function Berita() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const response = await getKioskBeritas();
                if (!active) {
                    return;
                }
                ...
                {loading ? (
                    <div className="flex gap-4 overflow-hidden">
                        {Array.from({ length: 4 }).map((_, index) => (
                            <div
                                key={index}
                                className="h-[340px] w-[75%] shrink-0 animate-pulse rounded-3xl bg-slate-100 sm:w-[45%] md:w-[30%] lg:w-[24%]"
                            />
                        ))}
                    </div>
                ) : beritas.length === 0 ? (
                    ...
                    >
                        {beritas.map((berita) => (
                            ...
                            >
                                <div className="relative h-[160px] overflow-hidden bg-[#075985]/10">
                                    {berita.image_url ? (
                                        <img
                                ...
                                <div className="p-5">
                                    ...
                                    <span className="mt-4 inline-flex items-center gap-1 text-[13px] font-semibold text-[#075985]">
                                        Baca Selengkapnya
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
