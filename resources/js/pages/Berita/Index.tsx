...
import { deleteBerita, getBeritas, type Berita } from '@/api/berita';
...
export default function BeritaIndex() {
    ...
    const loadBeritas = useCallback(async (targetPage = 1) => {
        try {
            setLoading(true);
            setError('');
            setPage(targetPage);

            const response = await getBeritas(targetPage);
            ...
            setError(
                error.response?.data?.message || 'Gagal mengambil data berita.',
            );
        } finally {
            setLoading(false);
        }
    ...
    const handleDelete = async (berita: Berita) => {
        const confirmed = window.confirm(
            `Hapus berita "${berita.title}"? Tindakan ini tidak bisa dibatalkan.`,
        );
    ...
    const filtered = beritas.filter((berita) => {
        ...
        return (
            berita.title.toLowerCase().includes(keyword) ||
            berita.slug.toLowerCase().includes(keyword) ||
            (berita.description ?? '').toLowerCase().includes(keyword)
        );
    });
                ...
                <div className="mt-4">
                    ...
                    <div className="flex h-12 items-center rounded-full border border-gray-200 bg-white px-4 shadow-sm">
                        ...
                        <input
                            ...
                            placeholder="Cari judul, slug, atau deskripsi berita..."
                    ...
                    {!loading && total > 0 && (
                        <div className="mt-4 flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm">
                            <p className="text-xs text-gray-400">
                                Menampilkan {beritas.length} dari {total} data
                            </p>
