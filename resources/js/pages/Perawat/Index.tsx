...
export default function PerawatIndex() {
    ...
    const loadPerawats = useCallback(async (targetPage = 1) => {
        try {
            setLoading(true);
            setError('');
            setPage(targetPage);

            const response = await getPerawats(targetPage);
    ...
    const handleDelete = async (perawat: Perawat) => {
        const confirmed = window.confirm(
            `Hapus perawat "${perawat.name}"? Tindakan ini tidak bisa dibatalkan.`,
        );
        ...
        try {
            setDeletingId(perawat.id);

            await deletePerawat(perawat.id);

            await loadPerawats(page);
        } catch (error: any) {
            console.error('Gagal menghapus perawat', error);

            window.alert(
                error.response?.data?.message || 'Gagal menghapus perawat.',
            );
        } finally {
            setDeletingId(null);
        }
    ...
    const filtered = perawats.filter((perawat) => {
        ...
        return (
            perawat.name.toLowerCase().includes(keyword) ||
            perawat.code.toLowerCase().includes(keyword) ||
            perawat.str_number.toLowerCase().includes(keyword)
        );
    });
                ...
                <div className="mt-4">
                    ...
                    <div className="flex h-12 items-center rounded-full border border-gray-200 bg-white px-4 shadow-sm">
                        ...
                        <input
                            ...
                            placeholder="Cari nama, kode, atau No. STR..."
                    ...
                    {!loading && total > 0 && (
                        <div className="mt-4 flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm">
                            <p className="text-xs text-gray-400">
                                Menampilkan {perawats.length} dari {total} data
                            </p>
