...
Stack: Laravel 13 (PHP 8.3) backend + Inertia v3 / React 19 SPA, session-based Sanctum auth, Spatie permissions. There is no `.ai/rules` directory and no repo-local OpenCode config (the boost block's project-rules step is a no-op here). Vite wrapper is `vite-plus` (`vp`), configured in `vite.config.ts` (not `.js`). CI runs in `.github/workflows/tests.yml`. The `README.md` is empty/stale — trust these notes and the code, not it.
...
## Known broken gates (verified, not caused by your diff)

As of the current tree, **all verified full gates are broken before they reach your code**. Do not chase these; fall back to the per-language checks below.

- **PHPStan cannot run** — `composer run test` and `composer run types:check` fail at bootstrap with `Undefined constant "Larastan\Larastan\LARAVEL_VERSION"` in `LarastanStubFilesExtension.php:25`. This is a Larastan 3.x / Laravel 13 environment incompatibility that fails before analyzing any code. Fall back to `vendor/bin/pint` + `vendor/bin/pest` for PHP verification.
- **`npm run check` fails** — `vp check` (lint gate, `denyWarnings: true`) reports 8 warnings in 135 files that are NOT auto-fixable and sit in files unrelated to typical diffs: `no-unused-vars` in `pages/Dashboard.tsx:36,40,41` and `pages/Kiosk/TicketDisplay.tsx:51`; `no-floating-promises` in `app.tsx:9`, `ssr.tsx:8`, `pages/Dashboard.tsx:44`, `pages/Kiosk/TicketDisplay.tsx:150`. Formatting itself passes ("All 143 files are correctly formatted"), so `npm run check --fix` will NOT clear these — only formatting issues.
- **`npm run types:check` fails** — `tsc --noEmit` errors on `resources/js/ssr.tsx:1`: `Module '"react-dom/server"' has no exported member 'createServer'`. Verified: neither `react-dom` 19.2.8 at runtime (exports `renderToString`, `renderToPipeableStream`, etc.) nor `@types/react-dom` exports `createServer`, so this scaffold entry is broken as committed. Use `tsc` output minus that known line, or manual review, for TS verification.
- Consequently `composer run ci:check` and the GitHub `tests` workflow cannot pass on the phpstan _and_ TS steps regardless of your diff. `vendor/bin/pint --test` is repo-wide; run `vendor/bin/pint` to fix files (or `--dirty` for just changes) to keep the PHP formatting gate green.
- Recommended local verification: `vendor/bin/pint` + `vendor/bin/pest --compact` for PHP; for TS, be aware the two gates above are pre-broken.

## Architecture & conventions

- API is versioned in `routes/api.php` under `Route::prefix('v1')->middleware(StartSession::class)`. This group has no `web` middleware, so the explicitly-added `StartSession::class` (`routes/api.php:26`) is what makes session auth work outside the web group — do not remove it.
    - **Public (no auth):** `auth/register`, `auth/login`, and the `kiosk` group (`kiosk/polis`, `kiosk/dokters`, `kiosk/beritas`, `kiosk/penawarans`, `kiosk/jadwal-dokters`, `kiosk/ruangans`, `kiosk/fasilitas`, `kiosk/tickets` POST, `kiosk/now-serving`, `kiosk/attendance/scan`).
    - **`auth:sanctum`:** `auth/me` GET/PUT, `auth/me/password`, `auth/logout`; apiResources for `fasilitas`, `polis`, `ruangans`, `pasiens`, `pendaftarans`, `antrians`, `beritas`, `penawarans`, `dokters`, `perawats`, `presensis`, `jadwal-dokters`, `pemeriksaans`, `obats`, `pembayarans`, `faqs`, `testimonials`, `messages`; custom ruangan sub-routes (`ruangans/{ruangan}/antrians`, `ruangans/{ruangan}/pasiens` POST/DELETE); and `monitoring` GET.
...
- Frontend: pages in `resources/js/pages` (per resource: `Index`/`Create`/`Edit`/`Show`, plus Kiosk, Dashboard, Monitoring, Auth, Profile, and public-facing `Dokter/Cari`, `Dokter/Detail`, `welcome`). `routes/web.php` holds Inertia closures — authenticated CRUD pages (dashboard, monitoring, every resource's Index/Create/Edit/Show, profile) are wrapped in `Route::middleware('auth')` so guests get redirected to `/login` server-side instead of seeing a page flash before the client-side 401 bounce. Public ones sit outside that group (`/`, `/cari-dokter`, `/dokter/{dokter:slug}` which 404s inactive dokters, `/fasilitas/{fasilitas:slug}` → `Fasilitas/Show`, `/register`, `/login`, `/ticket`, `/antrians-ticker`, `/absen-karyawan`). Query-string-driven pages: `Pendaftaran/Create` (`?antrian_id=N`), `Pemeriksaan/Create` (`?antrian_id=N&pasien_id=N&poli_id=N`), `Obat/Create` and `Pembayaran/Create` (`?pemeriksaan_id=N`), `Message/Edit` (`?reply=1`). Kiosk routes: `/ticket`, `/antrians-ticker`, `/absen-karyawan`.
...
- **Fasilitas ↔ Ruangan hierarchy**: `fasilitas` are top-level hospital units (UGD, IRJ, standard/advanced care, OK, Penunjang, Umum); `ruangans` are rooms/wards nested under a facility. `ruangans.facility_id` (nullable, FK `nullOnDelete`) replaced the old `category` string; `fasilitas` has no `category` at all. `Fasilitas::ruangans()` MUST pass the explicit FK: `hasMany(Ruangan::class, 'facility_id')` (default would guess `fasilitas_id`, which does not exist). The API resource for `fasilitas` is registered with `->parameters(['fasilitas' => 'fasilitas'])` — without it the wildcard singularizes to `{fasilita}` and route model binding resolves an empty model (update 500s on `route('fasilitas')`, delete silently no-ops). Ruangans expose `facility: {id, name, slug} | null` in `RuanganResource` (via `- select` and `facility?->name` in places like `PasienResource`), not `category`. Fasilitas has an optional `image` (stored to `public/fasilitas`, symlink required) exposed as `image_url` on `FasilitasResource`; Store/Update accept multipart `image` (nullable, `mimes:jpg,jpeg,png,webp`, max 2048) and `FasilitasService` deletes the old file on update/delete. The web show route `/fasilitas/{fasilitas:slug}` (renders `Fasilitas/Show`) is named **`fasilitas.detail`**, NOT `fasilitas.show` — naming it `fasilitas.show` collides with the API resource's `fasilitas.show` in generated Wayfinder types (see the `dokter.detail` precedent).
...
- **Absolute session expiry (24h):** Laravel's session GC only runs probabilistically after a request uses the cookie, so idle sessions with the `database` driver can live forever. `EnsureSessionFreshness` (alias `session.fresh`) enforces a hard logout. On login (`AuthService::login`) it stamps `logined_at` (unix ts) into the session; on every request it logs out once `now - logined_at >= session.lifetime * 60`. It is applied to the whole `web` middleware group and to the `auth:sanctum` API group (`routes/api.php`). Expiry returns 401 JSON for API/`expectsJson` requests (the React pages all redirect to `/login` on 401) and `redirect()->route('login')` for full page loads. `SESSION_LIFETIME=1440` (24h) is the knob in `.env`/`.env.example`; `logined_at === null` is treated as fresh (stamped now) so sessions started before the middleware don't break. Tests: `tests/Feature/SessionExpiryTest.php`.
